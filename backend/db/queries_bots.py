...
def admin_get_bot_logs(bot_key: Optional[str] = None, limit: int = 100) -> List[dict]:
    ...
    with get_db_connection() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            query = """
                SELECT 
                    l.log_id, EXTRACT(EPOCH FROM l.timestamp) as timestamp, 
        ...
        try:
            with conn.cursor(cursor_factory=DictCursor) as cursor:
                ...
                cursor.execute(
                    ...
                    """,
                    (public_key, uid, username, password_hash, "BOT_SYSTEM", 0, private_key, True, bot_type, prob, True)
                )
...
def admin_set_bot_config(public_key: str, action_probability: float) -> (bool, str):
    ...
    with get_db_connection() as conn:
        try:
            with conn.cursor(cursor_factory=DictCursor) as cursor:
                cursor.execute(
                    "UPDATE users SET action_probability = %s WHERE public_key = %s AND is_bot = TRUE",
...
def get_bot_type_configs() -> Dict[str, dict]:
    ...
    from backend.bots import BOT_LOGIC_MAP # 延迟导入
    configs = {}
